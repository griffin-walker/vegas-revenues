...
def load_tables(directory=DATA_DIRECTORY):
    ...
    for pdf in (directory / "csvs").iterdir():
        try:
            ...
            df = None
        month_year = "_".join(str(pdf).split("_")[0:2])
...
def get_clean_data_records(clean_data_map):
    games_of_interest = [
        "tables_twenty_one",
        "tables_craps",
        "tables_roulette",
...
def main():
    clean_data_map = load_tables()
    clean_records = get_clean_data_records(clean_data_map)
    clean_records = append_covid_months(clean_records)
    df = create_df(clean_records)
    df.to_csv("cleaned_vegas_revenues.csv", index=None)
